...
var Bam = Class.extend({
   ...
   init: function(bamUri, options) {
	  ...
	  var currentPort = 80;
      this.iobio.bamtools = "ws://" + currentHost + ":" + currentPort + "/bamtools/";
      this.iobio.samtools = "ws://" + currentHost + ":" + currentPort + "/samtools/";
      this.iobio.bamReadDepther = "ws://" + currentHost + ":" + currentPort + "/bamreaddepther/";
      this.iobio.bamMerger = "ws://" + currentHost + ":" + currentPort + "/bammerger/";      
      this.iobio.bamstatsAlive = "ws://" + currentHost + ":" + currentPort + "/bamstatsalive/";
      this.iobio.dcctool = "ws://" + currentHost + ":" + currentPort + "/dccstorageclient/"
...
//      this.iobio.bamMerger = "ws://localhost:8030";      
   ...
   getReferencesWithReads: function(callback) {
      var refs = [];
      var me = this;
      if (this.sourceType == 'url' || this.sourceType == 'dcc') {
      ...
      if ( Object.keys(me.readDepth).length > 0 )
         ...
         client.on('open', function(stream){
            ...
            stream.on('data', function(data, options) {
               ...
               //console.log ("last refresh time", now - this._lastUpdateTimer);
               //this._lastUpdateTimer = now; 
               data = data.split("\n");
               for (var i=0; i < data.length; i++)  {
                  if ( data[i][0] == '#' ) {
                     if ( Object.keys(readDepth).length > 0 ) { cb() };
