(function() {
  ...
  *******************************************************************************/
  ...
  ProjectMutationChart.prototype.render = function(element) {
    ...
    vis.append('g')
      .attr('transform', _this.translate(5, config.padding + config.chartHeight) + ' rotate(-90)')
      .append('text')
      .classed('axis-label', true)
      .text('Somatic mutation prevalence (Mb)');
    ...
    _this.data.forEach(function(projData, idx) {
      ...
      // Render labels
      labelX = (config.projectWidth / 2) + config.padding + (idx * config.projectWidth);
      labelY =  6 + config.padding + config.chartHeight;
      vis.append('text')
        .datum( { id: projData.id })
  ...
  ProjectMutationChart.prototype.highlight = function(ids) {
    ...
    d3.select(_this.element).selectAll('.axis-label').style('opacity', 1.0);
